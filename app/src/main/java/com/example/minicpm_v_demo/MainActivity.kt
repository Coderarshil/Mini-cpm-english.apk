...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupRecyclerView() {
        ...
        chatAdapter.setOnSuggestionClick { suggestion ->
            if (isModelReady) {
                ...
                Toast.makeText(this, "Please load the model first", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun clearChat() {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@MainActivity, "Failed to clear chat: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun loadDefaultModel() {
        ...
        // the image button leaves the user wondering why "Cannot open images 
        // in newly installed APK". Common trigger is `migrateLegacyLayoutIfNeeded` 
        // having just purged a stale mmproj after an APK upgrade - in that case 
        // the user needs to re-download from "Model Management".
    ...
    private fun promptDownloadModels(ggufMissing: Boolean, mmprojMissing: Boolean) {
        val message = when {
            ggufMissing && mmprojMissing ->
                "Model files not detected. Please go to 'Model Management' to download before use."
            mmprojMissing ->
                "This update includes a new image model (mmproj).\nPlease go to 'Model Management' to re-download, otherwise image recognition will not work."
            else ->
                "Incomplete model files. Please go to 'Model Management' to re-download."
        }
        AlertDialog.Builder(this)
            ...
            .setNegativeButton("Later") { _, _ ->
                Toast.makeText(
                    this,
                    "You can download at any time by clicking the 'Model Management' button in the top right corner",
                    Toast.LENGTH_LONG
                ).show()
            }
            .show()
    ...
    private fun handleSelectedMedia(uri: Uri) {
        ...
        when {
            ...
            mime.startsWith("image/") || mime.isEmpty() -> handleSelectedImage(uri)
                ...
                Toast.makeText(this, "Unsupported file type: $mime", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun handleSelectedImage(uri: Uri) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                } ?: throw RuntimeException("Failed to read image")
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@MainActivity, "Failed to process image: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun handleSelectedVideo(uri: Uri) {
        if (!engine.isVideoUnderstandingSupported) {
            Toast.makeText(this,
                "Video understanding is only available on MiniCPM-V-4.6, please go to 'Model Management' to switch models",
            ...
            try {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        if (index >= 0) {
                            ...
                            messages[index] = cur.copy(
                                imageInfo = "$info · Processing $current/$total"
                            )
                            chatAdapter.submitList(messages.toList())
                        }
                    }
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (index >= 0) {
                        val cur = messages[index] as ChatMessage.UserMessage
                        messages[index] = cur.copy(
                            imageInfo = "$info · Preprocessing ${elapsedMs / 1000.0}s",
                            isPrefilling = false
                        )
                        chatAdapter.submitList(messages.toList())
                    }
                ...
                withContext(Dispatchers.Main) {
                    ...
                    Toast.makeText(this@MainActivity, "Failed to process video: ${e.message}", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    ...
    private fun handleUserInput() {
        ...
        if (userMsg.isEmpty()) {
            Toast.makeText(this, "Please enter a text message", Toast.LENGTH_SHORT).show()
            return
        }
